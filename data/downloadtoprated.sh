...
do
   ...
   url="http://api.themoviedb.org/3/movie/top_rated?page=${i}&api_key=${api_key}"
   httpresponse=$(curl -o /dev/null --silent -w "%{http_code}" "$url")
    # ; #> movie${i}.json;
   #echo "$result"
   if [ $httpresponse -eq "200" ]
   then
      result=$(curl --silent --header "Accept: application/json" "http://api.themoviedb.org/3/movie/top_rated?page=${i}&api_key=${api_key}")
